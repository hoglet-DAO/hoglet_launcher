module hoglet_hodl::hodl_fa_config {
    use std::signer;
    use std::error;
    ...
    public entry fun transfer_emergency_admin(admin: &signer, new_admin: address) acquires GlobalConfig, AdminConfig {
        ...
        let global_config = borrow_global_mut<GlobalConfig>(@hoglet_hodl);
    ...
    public entry fun set_treasury_withdraw_grace_period(
        ...
        let config = borrow_global_mut<GlobalConfig>(@hoglet_hodl);
    ...
    public entry fun transfer_admin(
        ...
        let admin_config = borrow_global_mut<AdminConfig>(@hoglet_hodl);
    ...
    public fun get_emergency_admin_address(): address acquires GlobalConfig {
        borrow_global<GlobalConfig>(@hoglet_hodl).emergency_admin_address
    }

    #[view]
    public fun get_treasury_admin_address(): address acquires GlobalConfig {
        borrow_global<GlobalConfig>(@hoglet_hodl).treasury_admin_address
    }

    #[view]
    public fun get_treasury_withdraw_grace_period(): u64 acquires GlobalConfig {
        borrow_global<GlobalConfig>(@hoglet_hodl).treasury_withdraw_grace_period_seconds
    }

    #[view]
    public fun get_pool_registration_fee_config(): (u64, address) acquires GlobalConfig {
        let config = borrow_global<GlobalConfig>(@hoglet_hodl);
    ...
    public fun get_linked_contract_address(): Option<address> acquires GlobalConfig {
        borrow_global<GlobalConfig>(@hoglet_hodl).linked_contract_address
    }

    #[view]
    public fun is_global_emergency(): bool acquires GlobalConfig {
        borrow_global<GlobalConfig>(@hoglet_hodl).global_emergency_locked
    }

    #[view]
    public fun get_current_admin(): address acquires AdminConfig {
        borrow_global<AdminConfig>(@hoglet_hodl).current_admin
    }



    public fun assert_is_current_admin(admin_signer: &signer) acquires AdminConfig {
        assert!(exists<AdminConfig>(@hoglet_hodl), error::invalid_state(ERR_NOT_INITIALIZED));
        let admin_config = borrow_global<AdminConfig>(@hoglet_hodl);
    ...
    public fun assert_is_emergency_admin(admin_signer: &signer) acquires GlobalConfig {
        assert!(exists<GlobalConfig>(@hoglet_hodl), error::invalid_state(ERR_NOT_INITIALIZED));
        let global_config = borrow_global<GlobalConfig>(@hoglet_hodl);
